function [ file_list isdir numfiles ] = read_folder_contents( root_dir,extension, varargin )
% FUNCTION [ file_list isdir numfiles ] = READ_FOLDER_CONTENTS( root_dir,extension,varargin )
...
% Output:
% @ file_list:
%   This output argument is the list of files you are searching for.
% @ isdir:
%   This is a logical 1 or 0 whether this detected path is a file or not.
% @ numfiles:
%   This is the number of files detected with the parameters requested.
narginchk(1,4);
...
if ~exist('extension','var') || isempty(extension)
    ...
    elseif strcmp(varargin{1},'Prefix')
        file_list=dir(fullfile(root_dir,[varargin{2}, '*']));
    elseif strcmp(varargin{1},'Wildcard')
        file_list=dir(fullfile(root_dir,['*', varargin{2}, '*']));        
    end
else
    if isempty(varargin)
    ...
    elseif strcmp(varargin{1},'Prefix')
        file_list=dir(fullfile(root_dir,[varargin{2}, '*', extension]));
    elseif strcmp(varargin{1},'Wildcard')
        file_list=dir(fullfile(root_dir,['*', varargin{2}, '*', extension]));
    end
end
